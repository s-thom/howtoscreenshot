...
self.addEventListener("install", event => {
  ...
  const installation = caches
    .open(`${CACHE}-base`)
    .then(cache => cache.addAll(["/index.html", "/styles.css", "/script.js"]));
...
self.addEventListener("fetch", event => {
  console.log(
    `${LOGTAG} FETCHING ${event.request.url} (${
  ...
  let prom;
  if (CACHE_FIRST) {
    prom = fromCache(event.request).catch(() => fromNetwork(event.request));
  } else {
    prom = fromNetwork(event.request).catch(() => fromCache(event.request));
  }
...
self.addEventListener("activate", event => {
  ...
  event.waitUntil(
    caches
      ...
      .then(toDelete =>
        Promise.all(toDelete.map(cache => caches.delete(cache)))
      )
